...
async def main(args):
    ...
    chat_service = OllamaChatCompletion("ollama",
                                        host=args.ollama_address,
                                        ai_model_id=args.model_id)

    kernel.add_service(chat_service)
    async with MCPSsePlugin("JiraPlugin",
                            f"http://{args.mcp_address}/sse",
                            headers={
                                "Authorization": f"Basic {convert_credentials(
                                    args.mail, args.token
                                    )}"
                                }) as mcp_plugin:
        ...
        chat.add_system_message(
            "You are a helpful AI assistant that fulfils requests." +
            "You can also call a plugin for manipulating with Jira tickets." +
            "For calling these plugins, you do not need login information." +
            "If the user does not supply you with all the arguments,"
            " ask for this information before using the plugin. "
        ...
        while True:
            ...
            completion = service.get_streaming_chat_message_contents(
                chat,
                execution_settings,
                kernel=kernel
                )
