...
def get_space_id(name: str) -> Optional[str]:
    ...
    response = requests.get(
        f"https://{os.environ["space"]}.atlassian.net/wiki/api/v2/spaces",
        headers=headers)
...
def create_page(space: str, title: str, content: str) -> str:
    ...
    response = requests.post(
        f"https://{os.environ["space"]}.atlassian.net/wiki/api/v2/pages",
        headers=headers,
...
def register_tools(mcp: FastMCP):
    mcp.add_tool(
        search_page,
        description="""Searches for pages in the space by the title.
    ...
    mcp.add_tool(
        create_page,
        description="""Creates new Confluence page.
...
if __name__ == "__main__":
    ...
    mcp = FastMCP(name="ConfluenceServer")
    register_tools(mcp)
